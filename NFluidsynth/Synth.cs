...
namespace NFluidsynth
{
	...
	public class Synth : FluidsynthObject
	{
		...
		public Settings Settings {
			get { return new Settings (LibFluidsynth.fluid_synth_get_settings (Handle)); }
		}
		...
		{
			int ret;
			if (LibFluidsynth.fluid_synth_get_cc (Handle, channel, num, out ret) != 0)
		...
		public bool Sysex (byte [] input, byte [] output, bool dryrun = false)
		{
			...
			bool handled;
			if (LibFluidsynth.fluid_synth_sysex (Handle, input, input.Length, output, ref outlen, out handled, dryrun) != 0)
		...
		{
			int ret;
			if (LibFluidsynth.fluid_synth_get_pitch_bend (Handle, channel, out ret) != 0)
		...
		{
			int ret;
			if (LibFluidsynth.fluid_synth_get_pitch_wheel_sens (Handle, channel, out ret) != 0)
		...
		{
			if (LibFluidsynth.fluid_synth_program_select_by_sfont_name (Handle, channel, soundFontName, bank, preset) != 0)
		...
		{
			IntPtr info;
			if (LibFluidsynth.fluid_synth_get_channel_info (Handle, channel, out info) != 0)
		...
		{
			if (LibFluidsynth.fluid_synth_set_gen2 (Handle, channel, param, value, absolute, normalized) != 0)
		...
		public void CreateKeyTuning (int bank, int prog, string name, double [] pitch)
		{
			...
			if (LibFluidsynth.fluid_synth_create_key_tuning (Handle, bank, prog, name, pitch) != 0)
		...
		public void ActivateKeyTuning (int bank, int prog, string name, double [] pitch, bool apply)
		{
			...
			if (pitch.Length != 128)
				throw new ArgumentException ("pitch array must be of 128 elements.");
			if (LibFluidsynth.fluid_synth_activate_key_tuning (Handle, bank, prog, name, pitch, apply) != 0)
				OnError ("key tuning create operation failed");
		}
		...
		public void CreateOctaveTuning (int bank, int prog, string name, double [] pitch)
		{
			...
			if (pitch.Length != 128)
				throw new ArgumentException ("pitch array must be of 128 elements.");
			if (LibFluidsynth.fluid_synth_create_octave_tuning (Handle, bank, prog, name, pitch) != 0)
				OnError ("key tuning create operation failed");
		}
		...
		public void ActivateOctaveTuning (int bank, int prog, string name, double [] pitch, bool apply)
		{
			...
			if (pitch.Length != 128)
				throw new ArgumentException ("pitch array must be of 128 elements.");
			if (LibFluidsynth.fluid_synth_activate_octave_tuning (Handle, bank, prog, name, pitch, apply) != 0)
				OnError ("key tuning create operation failed");
		}
		...
		public void TuneNotes (int bank, int prog, int[] keys, double[] pitch, bool apply)
		{
			...
			if (pitch.Length != 128)
				throw new ArgumentException ("pitch array must be of 128 elements.");
			if (LibFluidsynth.fluid_synth_tune_notes (Handle, bank, prog, keys.Length, keys, pitch, apply) != 0)
				OnError ("key tuning create operation failed");
		}

		public void SelectTuning (int channel, int bank, int prog)
		{
			if (LibFluidsynth.fluid_synth_select_tuning (Handle, channel, bank, prog) != 0)
		...
		{
			if (LibFluidsynth.fluid_synth_reset_tuning (Handle, channel) != 0)
		...
		public double [] TuningDump (int bank, int prog, out string name)
		{
			...
			byte [] nm = new byte [64];
			LibFluidsynth.fluid_synth_tuning_dump (Handle, bank, prog, nm, nm.Length, ret);
		...
		public string LastError {
			get {
				var ptr = LibFluidsynth.fluid_synth_error (Handle);
		...
		{
			if (LibFluidsynth.fluid_synth_write_s16 (Handle, length, leftOut, leftOffset, leftIncrement, rightOut, rightOffset, rightIncrement) != 0)
		...
		{
			if (LibFluidsynth.fluid_synth_write_float (Handle, length, leftOut, leftOffset, leftIncrement, rightOut, rightOffset, rightIncrement) != 0)
		...
		{
			float [] dummy, dummy2;
			if (LibFluidsynth.fluid_synth_nwrite_float (Handle, length, out leftOut, out rightOut, out dummy, out dummy2) != 0)
				OnError ("float sample write operation failed");
		}

		public void Process (int length, int nIn, float [] inBuffer, int nOut, float [] outBuffer)
		{
			if (LibFluidsynth.fluid_synth_process (Handle, length, nIn, inBuffer, nOut, outBuffer) != 0)
				OnError ("float sample write operation failed");
		}
		...
		public bool GetVoiceList (Voice [] voices, int voiceId)
		{
			...
			LibFluidsynth.fluid_synth_get_voicelist (Handle, arr, arr.Length, voiceId);
		...
		{
			if (LibFluidsynth.fluid_synth_handle_midi_event (Handle, midiEvent) != 0)
				OnError ("midi event handle operation failed");
		}

		public void SetMidiRouter (IntPtr router)
		{
			LibFluidsynth.fluid_synth_set_midi_router (Handle, router);
		}
	}
}
